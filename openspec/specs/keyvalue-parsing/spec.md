# KeyValue Parsing Specification

> Synced from change keyvalue-tokenization-parsing on 2026-04-23

## Purpose

The parser converts `KeyValue` tokens emitted by the tokenizer into `.keyValue(key:value:)` `Expression` nodes. Key-value nodes compose with all existing boolean operators (`AND`, `OR`, `NOT`, parentheses) and coexist with `.contains` nodes. Backslash-escaped key-value patterns parse as `.contains` to preserve backward compatibility, and v1 queries without colons produce identical expression trees.
...
- **Dependencies**: KeyValue tokenization, `Expression` enum
